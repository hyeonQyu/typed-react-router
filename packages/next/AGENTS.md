...
**Mental model:** the route tree object is the single source of truth for the application's information architecture. You declare the whole IA once as a nested object whose keys mirror `src/app/` one-for-one; pathnames, path params, search-param types, navigation, and route metadata are all *derived* from that declaration and checked by the compiler. Writing a URL string by hand, or keeping a second list of routes anywhere, defeats the entire point of the library.
