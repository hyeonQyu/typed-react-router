...
/**
 * `useTypedSearchParams` builds on Next's `useSearchParams`, which opts a page out of
 * static prerendering — so Next requires a `<Suspense>` boundary around it, exactly as
 * it would for `useSearchParams` directly.
 */
    ...
    <>
      ...
      <Section title="useTypedSearchParams('/products')">
        ...
        <p style={{ fontSize: 13, color: '#6b7280', margin: '0.75rem 0 0' }}>
          ...
          <strong>number</strong> and <code>?inStock=true</code> as a <strong>boolean</strong>, because the schema says so.
          v1 returned the raw strings here while claiming otherwise in the types.
        </p>
      </Section>
      ...
      <Section title="Try it">
        ...
        <button type="button" style={{ ...subtleLink, border: 0, cursor: 'pointer' }} onClick={() => router.push('/products', { searchParams: { page: (searchParams.page ?? 1) + 1 } })}>
